function [output, gapStart, gapEnd] = makeGap(sig, gapLen, gapStart)
    %MAKEGAP Introduce a gap of zeros in a given signal
    %   [output, gapStart, gapEnd] = MAKEGAP(sig, gapLen) returns a copy of
    ...
    %
    %   [output, gapStart, gapEnd] = MAKEGAP(sig, gapLen, gapStart) returns
    ...
    output = sig;
    output(gapStart:gapStart + gapLen - 1) = zeros(gapLen, 1);
