classdef FindSpecPeaksTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testFreqEstAccuracy(testCase)
            l = 2048;

            for freq = linspace(50, 20000, 1000)
                sig = getCosSig(l, freq);
                [freqEst, ~, ~] = findSpecPeaks(sig, -Inf, 1);

                testCase.verifyEqual(freqEst, freq, 'RelTol', 0.001);
            end
        end
