function [magSpec, phsSpec, nfft] = getFT(sig, nfft)
    ...
    %   The signal is first windowed by a Hann function. The magnitude is
    %   returned in dBFS and corresponds to the magnitude of a sinusoid at
    %   a given bin frequency.
    %
    %   [magSpec, phsSpec, nfft] = GETFT(sig) uses optimal zero-padding to
    %   stay within the 0.1% tolerance
